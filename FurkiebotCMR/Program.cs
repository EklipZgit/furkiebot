			...
			get
			{
				lock (_instanceLock)
				{
					if (instance == null)
					{
						...
						conf.name = ConfigurationManager.AppSettings["BotName"];
						conf.nick = ConfigurationManager.AppSettings["BotName"];
						conf.altNick = ConfigurationManager.AppSettings["BotAltName"];
						conf.port = int.Parse(ConfigurationManager.AppSettings["IrcPort"]);
						conf.server = ConfigurationManager.AppSettings["IrcServerUrl"];
...
namespace FurkiebotCMR {
    ...
    public class FurkieBot : IDisposable {
        ...
        public const string MAP_PACK_LINK = @"http://redd.it/279zmi";
        public const string INFO_LINK = URL_BASE;
>>>>>>> 5af08c4f4ffbbeeebc208a721f646d29efb7239e
...
>>>>>>> 5af08c4f4ffbbeeebc208a721f646d29efb7239e
			...
			mainchannel = ConfigurationManager.AppSettings["MainChannel"]; //also the channel that will be joined upon start, change to #dustforcee for testing purposes
			cmrchannel = ConfigurationManager.AppSettings["CmrChannel"];
...
>>>>>>> 5af08c4f4ffbbeeebc208a721f646d29efb7239e
		...
		private bool ProcessInput(string[] input, string data, char[] charSeparator)
		{
			...
			if (chan.ToLower() == ConfigurationManager.AppSettings["BotName"].ToLower())
			{ //allows furkiebot to consider PM's as a "channel"
				chan = nick;
			}
...
>>>>>>> 5af08c4f4ffbbeeebc208a721f646d29efb7239e
			...
			switch (op)
			{
				...
				case "353": //NAMES command answer from server
					if (comNames == "CANCEL") //Kick all irc users from channel
					{
						...
						foreach (string s in name)
						{
							char[] gottaTrimIt = new char[] { '@', '+', '%' };
							string n = s.Trim().TrimStart(gottaTrimIt);
							if (n != ConfigurationManager.AppSettings["BotName"])
							{
								sendData("KICK", realRacingChan + " " + n);
							}
						}
				...
				case "JOIN": //Message someone when they join a certain channel
					...
					if (chan == ":" + mainchannel)
					{//Event: When someone joins main channel
						if (cmrStatus == "open")
						{//Message sent to someone that joins the main channel, notifying that there's a CMR open at the moment
							Notice(nick, "Entry currently " + ColourChanger("OPEN", "03") + " for Custom Map Race " + cmrId + ". Join the CMR at " + ColourChanger(realRacingChan, "04") + ". " + CountEntrants() + " entrants");
						}
						if (cmrStatus == "racing")
						{//Message sent to someone that joins the main channel, notifying that there's a CMR going on at the moment
							Notice(nick, "Custom Map Race " + cmrId + " is currently " + ColourChanger("In Progress", "12") + " at " + ColourChanger(realRacingChan, "04") + ". " + CountEntrants() + " entrants");
						}
					}
					break;
			...
			{//Commands without parameters
				...
				switch (command.ToLower())
				{
					...
					case ":.furkiebot": //FurkieBot Commands
						...
						{            // FurkieBot commands for race channel
					...
					case ":.startcmr": //Opening that CMR hype
						...
						if (cmrStatus == "closed")
						{
							...
							string nextCmrS = duration.Seconds.ToString();
							int mapCount = MapMan.GetAcceptedMaps().Count();
							int minMaps = int.Parse(ConfigurationManager.AppSettings["MinMaps"]);
							if (mapCount < minMaps && cmrtime.ToString(@"%h\:mm\:ss") == "10:30:00")
							{// If there are less than MIN_MAPS maps submitted AND if command wasn't issued using .startcmr+
								Msg(chan, "There are not enough maps to start a CMR. We need " + (minMaps - mapCount) + " more maps to start a CMR.");
							}
							else
							{
								TimeSpan stopTheTime = new TimeSpan(20, 29, 20);
						...
						if (chan == realRacingChan)
						{
							...
							if (cmrStatus == "open")
							{
								...
								if (UserMan.IsRegistered(nickLower))
								{
									...
									{
										if (!CheckEntrant(nickLower))
										{//Command only works if user isn't part of the race
											//Add user to race
											AddEntrant(nickLower);
											string extraS = "";
											if (CountEntrants() > 1)
					...
					case ":.unenter":
						if (chan.ToLower() == realRacingChan.ToLower())
						{
							...
							{
								//Command only works if CMR is open
								if (GetStatus(nickLower) == 6 || GetStatus(nickLower) == 3)
								{//Command only works if racer status is "standby" or "ready"
									//Remove user from race
									RemoveEntrant(nickLower);
									string extraS = "";
									if (CountEntrants() > 1 || CountEntrants() == 0)
							...
							{ //Command only works if CMR is open
								...
								{ //Command only works if there is at least 1 racer
									...
									{ //Command only works if all racers have status on "ready"
										...
										if (valid)
										{    // All maps have an atlas link recognized
											checker.StopChecking();
											cmrStatus = "racing";
											Msg(chan, BoldText(ColourChanger("The race will begin in 10 seconds!", "04")));
											Thread.Sleep(5000);
					...
					case ":.undone": //Not quite done or continue racing after quitting
						...
						{ //Command only works in racing channel
							...
							{ //Command only works if CMR is open
								if (GetStatus(nickLower) == 1 || GetStatus(nickLower) == 4)
								{ //Command only works if racer status is "done" or "quit"
									//Set racer status to "racing"
									SetStatus(nickLower, 2);
									Msg(chan, "" + nick + " isn't done yet.");
								}
			...
			{ //Commands with parameters
				...
				switch (command.ToLower())
				{
					...
					case ":.settester":
						...
						{
							if (UserMan.IsTrusted(nickLower, nick))
							{//trusted
								...
								else if (paramLower == "false")
								{
									...
									if (UserMan.IsAdmin(nickLower, nick))
									{ //admin wants to set himself to not-tester
										setTester(nickLower, parameter);
										Notice(nick, "As you wish, sir. You are no longer a tester.");
									}
									else
									{                        //normal tester wants to set himself to not-tester
					...
					case ":.setstream":
						...
						if (UserMan.IsAdmin(nickLower, nick) && splitStream.Length > 1)
						{  //admin is sending this command
					...
					case ":.setadmin":
						if (nickLower == "eklipz" || nickLower == "furkiepurkie")
						{ //TODO replace with fullops and halfops or someshit?
							char[] separator = { ' ' };
							string[] split = parameter.Split(separator, 2);
		...
		private void OutputCMRinfo(string chan, TimeSpan cmrtime, string cmrtimeString, string nickname)
		{
			...
			string nextCmrS = duration.Seconds.ToString();
			int minMaps = int.Parse(ConfigurationManager.AppSettings["MinMaps"]);
			if (CmrAcceptedCount(cmrId) < minMaps)
			{
				//If there are less than 6 maps submitted
				Msg(chan, "Upcoming race is Custom Map Race " + cmrId + ". There are only " + MapMan.GetAcceptedMaps().Count() + " maps currently accepted, and we need at least " + minMaps + ".");
		...
		{
			int allowedPastas = 2;
			if (nick.ToLower() != lastPastaer.ToLower())
			{ //reset the slap limit because someone else slapped.
				lastPastaer = nick;
				repeatPastas = 0;
			}
			else
			{    //increment slap count from same user.
				repeatPastas++;
			}
		...
		private void Slap(string nickname, string chan, string nameToSlap)
		{
			...
			if (repeatSlaps < allowedSlaps && split.Length < 4 && nameToSlap.Length < 30)
			{
				...
				if (toSlap != null)
				{ //get the targets name properly capitalized, if it is in our DB.
					nameToSlap = toSlap.Name;
				}

				if (nameToSlap.ToLower() == "glados")
				{      //they told furkiebot to slap GLaDOS
				...
				{  //person is trying to slap themselves.
					Msg(chan, ACT + @"ACTION uses " + nickname + "'s own hands to slap them. \"STOP HITTING YOURSELF, STOP HITTING YOURSELF!\"" + ACT);

				}
				else if (UserMan.IsAdmin(nameToSlap.ToLower(), nickname))
				{   //trying to slap an admin
				...
				{    //proceed with normal slap handling.
					if (nameToSlap.ToLower() == "furkiebot")
					{      //they told furkiebot to slap himself, slap them.
						nameToSlap = nickname;
		...
		/// <returns></returns>
		private bool AddEntrant(string racer)
		{//Used to add a racer to entrants
		...
		static int GetCurrentCMRidFromFile()
		{//Used to fetch the current CMR number
		...
		static string GetCurrentCMRStatus()
		{//Used to fetch current CMR status
		...
		static void SetCurrentCMRStatus(string s)
		{//Used to either open or close a CMR
			string text = s;
		...
		static string GetCMRMaps(string cmrid, DataTable dt)
		{//Used to get a certain line from the cmrmaps.txt file
			string res = "";
			...
			{
				if (!firstMap)
				{ // Makes sure seperator doesn't get placed before the first place
		...
		{ //Used to count the amount of lines in a certain file
			long count = 0;
		...
		static string GetTime(Stopwatch s)
		{ //Fetches current time on stopwatch
		...
		private void StartRace(DataTable racers, Stopwatch s)
		{ //Starts running timer
			if (CountEntrants() > 0)
			{
				s.Start();
		...
		{ //Used to count the amount entrants in the current CMR
			return racers.Rows.Count;
		} /* CountEntrants() */



		private string GetEntrantString()
		{ //Used to get one single string of entrants
			string result = "";
			if (CountEntrants() > 0)
			{
				DataView dv = racers.DefaultView;
				...
				for (int i = 0; i < CountEntrants(); i++)
				{
					...
					if ((i + 1) != CountEntrants())
					{ //If currentracer isnt the last one on the list add a "|" and start over
						currentracer += " | ";
					}
					result += currentracer;
				}
		...
		private bool CheckEntrant(string racer)
		{ //Checks if a certain user has entered the race
		...
		{
			int status = 0;
			if (CheckEntrant(racer))
			{ //If racer exists in race
				for (int i = 0; i < CountEntrants(); i++)
				{
					string name = racer;
		...
		private bool ComfirmMassStatus(int status)
		{ //Checks if the whole list of racers share the same status
			bool get = true;
			if (CountEntrants() > 0)
			{
				for (int i = 0; i < CountEntrants(); i++)
		...
		private bool ComfirmDoubleMassStatus(int s1, int s2)
		{ //Checks if the whole list of racers share the same status
			bool get = true;
			if (CountEntrants() > 0)
			{
		...
		private bool ComfirmTripleMassStatus(int s1, int s2, int s3)
		{ //Checks if the whole list of racers share the same status. 
			bool get = true;
			if (CountEntrants() > 0)
			{
		...
		private bool SetStatus(string racer, int newStatus)
		{ //Sets status of a racer
			var foundRows = racers.Select("Name = '" + racer + "'");
			if (foundRows.Length != 0)
			{
				foreach (DataRow dr in racers.Rows)
		...
		private bool SetTime(string racer, Stopwatch timer)
		{ //Sets time on a racer that .done
			if (CheckEntrant(racer))
			{ //If user exists in race
				foreach (DataRow dr in racers.Rows)
				{
		...
		private string GetRanking(string racer)
		{ //Used to get proper ranks like 1st, 2nd, 3rd etc.
			int r = 0;
			DataView dv = racers.DefaultView;
			dv.Sort = "Status, Hour, Min, Sec, TSec";
			racers = dv.ToTable();
		...
		/* MIRC COLOURS           
			...
			*/
		public static string ColourChanger(string s, string colour)
		{ //Used to colourcode text in irc
		...
		static string GetCurrentDateTime(int id)
		{ //1 = Day; 2 = 24time; 3 = 12time
			string res = "";
		...
		static string JsonToDatatableMaps2(DataTable dt, string cmrid, string irccommand, string ircchannel)
		{
			...
			if (File.Exists(DATA_PATH + @"CMR Results\CMR" + cmrid + "Results.json"))
			{ //Check if CMR number exists // !! FILEPATH !!
		...
		public static string FormatStreamURL(string streamURL)
		{
			...
			string finalURL = "";
			if (streamLower.StartsWith(@"http://") || streamLower.StartsWith(@"https://"))
